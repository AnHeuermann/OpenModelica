...
void setGlobalLoggingTime(int argc, char**argv, SIMULATION_INFO *simulationInfo)
{
  ...
  if (flagStr==NULL || *flagStr=='\0')
  {
    ...
    simulationInfo->settings.useLoggingTime = 0;
    return;
  }
  ...
  /* Save logging time */
  simulationInfo->settings.useLoggingTime = 1;
  simulationInfo->settings.loggingTimeRecord[0] = loggingStartTime;
  simulationInfo->settings.loggingTimeRecord[1] = loggingStopTime;
  infoStreamPrint(LOG_STDOUT, 0, "Time dependent logging enabled. Activate loggin in intervall [%f, %f]", simulationInfo->settings.loggingTimeRecord[0], simulationInfo->settings.loggingTimeRecord[1]);
