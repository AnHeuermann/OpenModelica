...
CMD="$@"
FMU_DIR="MakeCrossCompile"

echo "$VERSION"
echo "Testing FMUs from $FMU_DIR"

for fmu in $FMU_DIR/*.fmu; do
  fmu=`echo $fmu | sed 's/[.]fmu$//' | sed "s|${FMU_DIR}/||"`
  for t in me cs; do
    if test "$t" = "me"; then
      d="ModelExchange"
    else
